(function() {
    ...
    let lastConnectedRelays = [];       // track which relays successfully connected
    ...
    window.loadFeed = async function() {
        ...
        try {
            ...
            // Track which relays actually connected
    ...
    async function loadOlderPosts() {
        ...
        if (lastConnectedRelays.length === 0) {
            console.warn('No connected relays available for older posts');
            return;
        }
    ...
    async function likePost(postId, postPubkey, postKind) {
        if (typeof currentUser === 'undefined' || !currentUser) { window.showToast('Please login first.', 'info'); if (typeof window.showLoginModal === 'function') window.showLoginModal(); return; }
