(function() {
    ...
    function logout() { currentUser = null; clearLogin(); updateUserUI(); cachedProfile = null; renderMyProfile(); showToast('Logged out.', 'info'); }

    // ── Account Modal ──
    function showAccountModal(forceRefresh = false) {
        if (!currentUser) return;
        if (forceRefresh || !cachedProfile) {
            fetchAndCacheProfile().then(() => renderAccountModal(cachedProfile.profile, cachedProfile.profileEvent));
        } else {
    ...
    // ── Thread View (with boost button for logged-in users) ──
    ...
    async function runAnalysis(inputValue) {
        ...
        // Handle naddr (replaceable event coordinate)
        if (parsed.source === 'naddr') {
            ...
            tempInvestigator.onComplete = (inv) => {
                if (inv.events.length > 0) {
                    const event = inv.events[0];
                    runAnalysis(event.id);
                } else {
            ...
            await tempInvestigator.rm.connectAll(CONFIG.relayConnectTimeout);
            tempInvestigator.rm.subscribe([filter]);
            return;
        }
    ...
    function renderAll(inv) {
        ...
        ['timeline','stats','json','relays','bch','profile'].forEach(id => {
            const el = document.getElementById('panel-'+id); if(el) el.classList.remove('active');
        });
    }
    ...
    function initApp() {
        ...
        } else { setTimeout(initApp, 200); return; }
