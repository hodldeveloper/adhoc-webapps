(function() {
    ...
    function parseInput(input) { const t = input.trim(); if (!t) return { error: 'Please enter an event or user identifier.' }; if (
        ...
        if (t.startsWith('nevent1')) { const r = decodeNevent1(t); if (r && r.eventId) return { hexId: r.eventId,
                source: 'nevent1', relayHints: r.relayHints || [] }; return { error: 'Invalid nevent1 identifier.' }; } if (
            t.startsWith('npub1')) { const pubkey = decodeNpub(t); if (pubkey) return { pubkey, source: 'npub' }; return { error: 'Invalid npub identifier.' }; }
        if (t.startsWith('nprofile1')) { const r = decodeNprofile(t); if (r && r.pubkey) return { pubkey: r.pubkey,
                source: 'nprofile', relayHints: r.relayHints || [] }; return { error: 'Invalid nprofile identifier.' }; }
        const nostrUri = t.match(
            /^nostr:(note1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+|nevent1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+|npub1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+|nprofile1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+)$/i); if (
    ...
    function decodeNpub(s) { const d = bech32Decode(s); if (!d || d.hrp !== 'npub' || d.bytes.length !== 32) return null;
        return bytesToHex(d.bytes); }
    function decodeNprofile(s) { const d = bech32Decode(s); if (!d || d.hrp !== 'nprofile' || d.bytes.length < 32) return null;
        const pubkey = bytesToHex(d.bytes.slice(0, 32)); const tlvs = []; let idx = 32; while (idx < d.bytes.length) { if (
            ...
            new Uint8Array(t.value))); return { pubkey, relayHints: hints }; }
    ...
    class UserProfileInvestigator { constructor(relayManager) { this.rm = relayManager;
            ...
            this.follows = [];
            this.relays = [];
            this.profileEvent = null; } async investigate(pubkey, hints = []) { const allRelays = [...new Set([...
                    ...activeRelays, ...hints
                ])];
            this.rm.relayUrls = allRelays;
            this.rm.connections.clear();
            ...
            showLoading('Connecting to relays...'); await this.rm.connectAll(10000); const connected = [...relayStats
                ...
                showToast('No relays connected.', 'error'); return; }
            showLoading(`Fetching profile for ${npubFromHex(pubkey).substring(0,12)}...`);
            this.profile = null;
            this.follows = [];
            this.relays = [];
            this.profileEvent = null; const pending = new Set(); const profileSub = this.rm.subscribe([{ kinds: [0],
                authors: [pubkey], limit: 1 }]); const followsSub = this.rm.subscribe([{ kinds: [3],
                authors: [pubkey], limit: 1 }]); const relaySub = this.rm.subscribe([{ kinds: [10002],
                authors: [pubkey], limit: 1 }]);
            pending.add(profileSub);
            pending.add(followsSub);
            pending.add(relaySub);
            this.rm.onEvent = (ev) => { if (ev.pubkey === pubkey) { if (ev.kind === 0) { try { this.profile = JSON.parse(
                                ev.content || '{}'); } catch (e) { this.profile = {}; }
                        this.profileEvent = ev; } if (ev.kind === 3) this.follows = ev.tags.filter(t => t[0] === 'p').map(
                            t => t[1]); if (ev.kind === 10002) this.relays = ev.tags.filter(t => t[0] === 'r').map(t => t[
                            1]); } }; return new Promise((resolve) => { this.rm.onEOSE = (subId) => { pending.delete(
                            subId); if (pending.size === 0) { this.rm.onEvent = null;
                            this.rm.onEOSE = null;
                            resolve(); } };
                    setTimeout(() => { for (const sid of pending) this.rm.closeSubscription(sid);
                        pending.clear();
                        this.rm.onEvent = null;
                        this.rm.onEOSE = null;
                        resolve(); }, 10000); }).then(() => { hideLoading(); if (!this.profile && this.follows
                        .length === 0 && this.relays.length === 0) showToast('No profile found.', 'info'); else showToast(
    ...
    function renderMediaFromContent(content) { if (!content) return { text: '', media: '' }; const urlRegex =
        ...
        let match; while ((match = urlRegex.exec(content)) !== null) { const url = match[0]; if (/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i
                .test(url)) { mediaHtml +=
                `<img src="${url}" alt="Image" loading="lazy" style="max-width:100%;max-height:200px;border-radius:8px;display:block;margin:6px 0;" onerror="this.style.display='none'">`; } else if (/\.(mp4|webm|ogg)(\?.*)?$/i
                .test(url)) { mediaHtml +=
                `<video controls preload="metadata" style="max-width:100%;max-height:200px;display:block;margin:6px 0;"><source src="${url}" type="video/mp4"></video>`; } }
    ...
    function renderTimeline(inv) { const p = document.getElementById('panel-timeline'); const sorted = [...inv.events].sort((
        ...
        sorted.forEach(e => { const time = new Date((e.created_at || 0) * 1000).toLocaleTimeString([], { hour: '2-digit',
                minute: '2-digit' }); const kind = KNOWN_KINDS[e.kind] || `Kind ${e.kind}`; const isOrig = e.id ===
                investigationHexId; const { text, media } = renderMediaFromContent(e.content);
    ...
    function renderStats(inv) { const p = document.getElementById('panel-stats'); const tree = inv.getThreadTree();
        let nested = 0; if (tree && tree.childrenMap) { const count = (eid, d) => { let c = 0; for (const child of (tree
            ...
            nested = count(tree.rootId, 0); } const stats = [{ l: 'Original', v: originalEvent ? 1 : 0 }, { l: 'Replies',
                v: inv.getEventsByKind(1).filter(e => e.id !== investigationHexId && inv.getParentIds(e).includes(
                    investigationHexId)).length }, { l: 'Nested', v: nested }, { l: 'Quotes', v: inv.events.filter(e => e
                .kind === 1 && e.content && e.content.includes(investigationHexId || '') && !inv.getParentIds(e)
                .includes(investigationHexId || '')).length }, { l: 'Mentions', v: inv.events.filter(e => e.tags && e.tags
                .some(t => t[0] === 'e' && t[1] === investigationHexId)).length }, { l: 'Reposts', v: inv.getEventsByKind(
                6).length }, { l: 'Reactions', v: inv.getEventsByKind(7).length }, { l: 'Zaps', v: inv.getEventsByKind(
                9735).length + inv.getEventsByKind(9734).length }, { l: 'BCH Tips', v: inv.getBchPaymentEvents().length },
            { l: 'Unknown', v: inv.getUnknownEvents().length }, { l: 'Authors', v: inv.getUniqueAuthors() }, { l: 'Relays',
                v: [...relayStats.values()].filter(s => s.status === 'connected').length }, { l: 'Success', v: [
                    ...relayStats.values()
                ].filter(s => s.events > 0).length }, { l: 'Failed', v: [...relayStats.values()].filter(s => s.status ===
                    'failed' || s.status === 'disconnected').length }, { l: 'Images', v: inv.getMediaCounts().images },
            { l: 'Videos', v: inv.getMediaCounts().videos }, { l: 'Files', v: inv.getMediaCounts().attachments }, { l: 'Hashtags',
                v: inv.getHashtags() }, { l: 'Links', v: inv.getLinks() }, { l: 'Total', v: inv.events.length }, ];
        let h =
    ...
    function renderJson(inv) { const p = document.getElementById('panel-json'); let h =
            ...
            '</div><button class="btn btn-sm btn-outline" onclick="window._copyEventJson(\'' + originalEvent.id +
        ...
        h += '<h4 style="margin:16px 0 8px;">All Events (' + inv.events.length +
            ')</h4><input type="text" placeholder="Search JSON..." style="width:100%;padding:8px;background:var(--surface2);border:1px solid var(--border);color:var(--text);border-radius:8px;margin-bottom:8px;font-family:var(--mono);font-size:0.8rem;" oninput="window._searchJson(this.value)"><div class="json-viewer" style="max-height:50vh;">'; for (const e of inv
    ...
    function renderRelays() { const p = document.getElementById('panel-relays'); let h =
            ...
            h +=
            `<tr><td style="word-break:break-all;"><code style="font-size:0.65rem;">${escapeHtml(url)}</code></td><td><span class="status-dot ${cls}"></span>${txt}</td><td>${rt}</td><td>${s.events || 0}</td><td>${s.errors || 0}</td><td><button class="btn btn-sm btn-outline" onclick="window._reconnectRelay('${escapeHtml(url)}')">↻</button></td></tr>`; });
    ...
    function renderBch(inv) { const p = document.getElementById('panel-bch'); const evs = inv.getBchPaymentEvents(); if (!evs
        ...
        evs.forEach(e => { const sender = e.pubkey ? e.pubkey.substring(0, 12) + '...' : '?'; const recipient = e.tags ? (e
            ...
            h +=
            `<div class="bch-card" style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:8px;"><div><strong>Type:</strong> <span class="badge badge-orange">${e.paymentType}</span> | ${new Date((e.created_at||0)*1000).toLocaleString()}</div><div>${sender} → ${recipient}</div><div>Amount: ${amount} ${curr}</div>${txid!=='N/A'?`<div>TXID: <code style="word-break:break-all;">${txid}</code> <a href="https://blockchair.com/bitcoin-cash/transaction/${txid}" target="_blank" style="color:var(--blue);">🔗 Explorer</a></div>`:''}<div>Memo: ${escapeHtml((e.content||'').substring(0,200))}</div><button class="btn btn-sm btn-outline" onclick="window._inspectEvent('${e.id}')">View JSON</button></div>`; });
        h += '</div>';
        p.innerHTML = h; }

    // ── Profile Tab (scanned user) ─────
    function renderProfileTab(data, pubkey) { const p = document.getElementById('panel-profile'); const profile = data
            .profile || {}; let html =
            '<div class="card"><div class="card-header"><span class="card-title">👤 User Profile</span></div>';
        html += `<p><strong>npub:</strong> <code>${npubFromHex(pubkey)}</code></p>`; if (profile.name) html +=
            `<p><strong>Name:</strong> ${escapeHtml(profile.name)}</p>`; if (profile.about) html +=
            `<p><strong>About:</strong> ${escapeHtml(profile.about)}</p>`; if (profile.picture) html +=
            `<p><img src="${profile.picture}" alt="Profile" style="max-width:80px;border-radius:50%;"/></p>`; if (data.follows
            .length) html +=
            `<p><strong>Follows (${data.follows.length}):</strong> ${data.follows.map(f => `<code>${f.substring(0,8)}...</code>`).join(', ')}</p>`; if (
            data.relays.length) html +=
            `<p><strong>Relays:</strong> ${data.relays.map(r => `<code>${escapeHtml(r)}</code>`).join(', ')}</p>`; if (!profile
            .name && !profile.about && !profile.picture && !data.follows.length && !data.relays.length) { html +=
    ...
    async function investigateUser(pubkey, hints = []) { const allUrls = [...new Set([...activeRelays, ...hints])]; const rm =
            new RelayManager(allUrls);
        window._relayManager = rm; const upi = new UserProfileInvestigator(rm); await upi.investigate(pubkey,
        hints);
    ...
    async function runAnalysis(inputValue) { const input = inputValue || homeSearchInput.value.trim(); if (!input) {
            showError('Please enter an event or user identifier.'); return; }
        hideError(); const parsed = parseInput(input); if (parsed.error) { showError(parsed.error);
            showToast(parsed.error, 'error'); return; } if (parsed.pubkey) { await investigateUser(parsed.pubkey,
            parsed.relayHints || []); return; }
    ...
    function renderAll(inv) { allEvents = inv.events;
        ...
        window._investigationHexId = investigationHexId; if (allEvents.length === 0 && !originalEvent) { resultsScreen
                .classList.remove('active');
            homeScreen.classList.add('active'); return; }
        resultsScreen.classList.add('active');
        homeScreen.classList.remove('active');
        renderThread(inv);
        renderTimeline(inv);
