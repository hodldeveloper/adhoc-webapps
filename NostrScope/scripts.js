(function() {
    ...
    function showLoginModal() {
        ...
        if (typeof NostrTools === 'undefined') {
            safeToast('Nostr tools not loaded. Please refresh.', 'error');
            return;
        }
        ...
        if (!backdrop || !nsecInput || !confirmBtn || !cancelBtn) return;
        ...
        confirmBtn.addEventListener('click', async () => {
            ...
            let publicKey;
            try { publicKey = NostrTools.getPublicKey(privateKey); if (!isValidHex64(publicKey)) throw new Error('Invalid public key'); }
            catch (e) { safeToast('Unable to derive public key. Check your nsec.', 'error'); return; }
    ...
    // ── Analysis: full thread/timeline/stats/JSON/relays/BCH rendering ──
