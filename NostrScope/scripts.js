(function() {
    ...
    let relayManager = null, investigator = null, currentUser = null;
    ...
    function parseInput(input) {
        ...
        if (t.startsWith('nevent1')) {
            ...
            if (r && r.eventId) return { hexId: r.eventId, source: 'nevent1', relayHints: r.relayHints || [] };
            return { error: 'Invalid nevent1 identifier.' };
        }
        if (t.startsWith('npub1') || t.startsWith('nprofile1')) return { error: 'Profile identifier. Use event ID (note/nevent/hex).' };
        const nostrUri = t.match(
            /^nostr:(note1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+|nevent1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+)$/i);
        ...
        m = t.match(/https?:\/\/primal\.net\/e\/(note1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+)/i);
        if (m) return parseInput(m[1]);
    ...
    function decodeNevent1(s) {
        ...
        const hints = tlvs.filter(t => t.type === 1 || t.type === 2).map(t => new TextDecoder().decode(
            new Uint8Array(t.value)));
            ...
            return new Promise((res, rej) => {
                try {
                    ...
                    ws.onclose = () => {
                        ...
                        if (cur.status === 'connecting') relayStats.set(url, { ...cur, status: 'failed', errors: 1 });
                        else if (cur.status === 'connected') relayStats.set(url, { ...cur, status: 'disconnected' });
    ...
    class EventInvestigator {
        ...
        async investigate(hexId, hints = []) {
            ...
            setTimeout(() => {
                if (this.pendingSubs.size > 0) {
                    for (const s of this.pendingSubs) this.rm.closeSubscription(s);
                    this.pendingSubs.clear();
                    this.onAllEOSE();
                }
            }, 15000);
        ...
        addEvent(ev) {
            ...
            this.events.push(ev);
            if (ev.id === this.hexId && !this.originalEvent) this.originalEvent = ev;
        ...
        onAllEOSE() {
            this.allDone = true;
            hideLoading();
            const msg = this.originalEvent ? `Investigation complete: ${this.events.length} events found.` :
                (this.events.length > 0 ? `Original not found, but ${this.events.length} related events.` :
                    'No events found.');
        ...
        getMediaCounts() {
            ...
            for (const e of this.events) {
                ...
                if (e.content) {
                    const m = e.content.match(/https?:\/\/[^\s]+\.(jpg|jpeg|png|gif|webp|svg)/gi);
                    if (m) im += m.length;
        ...
        getHashtags() {
            const s = new Set();
            for (const e of this.events) {
                if (e.tags) for (const t of e.tags) if (t[0] === 't' && t[1]) s.add(t[1].toLowerCase());
                if (e.content) {
        ...
        getBchPaymentEvents() {
            ...
            for (const e of this.events) {
                ...
                else if (e.tags && e.tags.some(t => t[0] === 'cashtoken' || t[0] === 'bch' || t[0] === 'txid'))
                    res.push({ ...e, paymentType: 'bch_payment' });
                else if (e.content && /\b(bch|bitcoincash|cashtoken)\b/i.test(e.content) &&
    ...
    backdrop.querySelector('#loginConfirmBtn').addEventListener('click', () => {
        ...
        try {
            // First try official nip19 decoder
            let privateKey;
            try {
                ...
                console.warn('nostr.nip19.decode failed, trying custom Bech32 decoder:', nip19Error);
                // Fallback to our own Bech32 decoder (handles many edge cases)
            ...
            console.error('Nsec login error:', e);
    ...
    async function runAnalysis(inputValue) {
        ...
        if (!input) { showError('Please enter an event identifier.'); return; }
        hideError();
