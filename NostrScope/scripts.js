(function() {
    ...
    // ── Global login handler (called from inline onclick) ──
    window.processNsecLogin = function() {
        console.log('🟢 processNsecLogin CALLED');
        const nsecInput = document.getElementById('nsecInput');
        if (!nsecInput) {
            console.error('❌ nsecInput element not found!');
            safeToast('Internal error. Please refresh.', 'error');
            return;
        }
        ...
        try {
            ...
            if (!decoded || decoded.hrp !== 'nsec' || decoded.bytes.length !== 32) {
                safeToast('Invalid nsec format.', 'error');
                return;
            }
        ...
        if (!/^[0-9a-fA-F]{64}$/.test(privateKey)) {
            safeToast('Invalid private key.', 'error');
            return;
        }
        ...
        try {
            ...
            if (!isValidHex64(publicKey)) throw new Error('Invalid pubkey');
        } catch (e) {
            safeToast('Cannot derive public key.', 'error');
            return;
        }
    ...
    // ── Login Modal (uses inline onclick) ──
    function showLoginModal() {
        console.log('🔐 Opening login modal...');
        if (typeof NostrTools === 'undefined') {
            safeToast('Nostr tools not loaded.', 'error');
            return;
        }
        ...
        // Focus the input
        setTimeout(() => {
            const inp = document.getElementById('nsecInput');
            if (inp) inp.focus();
        }, 200);
    }
    ...
    function renderMyProfile() {
        ...
        if (!currentUser) {
            profileContent.innerHTML = `<div style="padding:20px;text-align:center;">
                ...
                <button class="btn btn-primary" id="loginFromProfileBtn" style="padding:10px 20px;background:#1d9bf0;color:#fff;border:none;border-radius:8px;font-weight:600;cursor:pointer;" onclick="window.showLoginModal();">🔑 Login</button>
            </div>`;
            return;
        }
        if (!cachedProfile) {
    ...
    window.processNsecLogin = window.processNsecLogin; // already set above, just ensuring
